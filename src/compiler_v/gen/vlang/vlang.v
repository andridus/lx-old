...
fn (mut g VGen) write_fns_node(modl string, nodes []ast.Node) {
	...
	if nodes.len > 0 {
		...
		for n in nodes {
			...
			// println(arity)
			g.write_fn_node(modl, n, arity_idx, fdata, arity, fns0.arities.len)
		}
	}
}
...
fn (mut g VGen) write_fn_node(modl string, node ast.Node, arity_idx int, fdata types.Function, arity table.FnArity, total_arities int) {
	...
	if total_arities > 0 {
		...
		g.writeln(modl, 'void *${module_name}_${fun_name}_0(){')
	}
	if fdata.arity != '0' {
		for arg0 in fdata.args {
			mut var_name := '${arg0.name}'
	...
	for n in node1.nodes {
		if current + 1 == total && node.meta.ti.kind == .void_ {
			g.parse_node(modl, n)
			g.writeln(modl, 'return Nil{}')
		} else if current + 1 == total && node.meta.ti.kind != .void_ {
			g.is_last_statement = true
			if !is_defer_return(n.meta.ti.kind) {
				if types.is_literal(n.kind) {
					tmp_ := g.temp_var(modl, n.meta.ti)
